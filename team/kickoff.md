...
that sets the lead and verifier; the IC agents default to `model: sonnet` in their
`.claude/agents/*.md` frontmatter, editable per role there.

## Step 0 — fill the project profile (automated)

Run once after install. This one is a plain prompt, not a shell command: open a
Claude Code session in the repo (plain `claude`) and paste it. It fills
`.claude/team/project-profile.md` from repo evidence and ends with the questions
only a human can answer — review the Data safety section before going further.
